...
public class Limelight extends SubsystemBase {
  ...
  public Limelight() {
    ...
    LimelightHelpers.setCameraPose_RobotSpace(
        ...
        Units.radiansToDegrees(leftRot.getX()),
        Units.radiansToDegrees(leftRot.getY()),
        Units.radiansToDegrees(leftRot.getZ()));
    LimelightHelpers.setCameraPose_RobotSpace(
        limelightRightName,
        Units.inchesToMeters(5),
        Units.inchesToMeters(7.4),
        Units.inchesToMeters(25.5),
        Units.radiansToDegrees(rightRot.getX()),
        Units.radiansToDegrees(rightRot.getY()),
        Units.radiansToDegrees(rightRot.getZ()));
  }

  public LimelightHelpers.PoseEstimate getRobotPose(String name, Drive driveTrain) {
    double robotYaw = driveTrain.getPose().getRotation().getDegrees();
  ...
  public void updateOdo(Drive driveTrain) {
  ...
  public void updateDisabledOdo(Drive driveTrain) {
    if (!robotHeadingReset) {
      ...
      if (leftPose != null && leftPose.tagCount > 0) {
        driveTrain.setPose(leftPose.pose);
        driveTrain.addVisionMeasurement(leftPose.pose, leftPose.timestampSeconds);
      }
      if (rightPose != null && rightPose.tagCount > 0) {
        driveTrain.setPose(rightPose.pose);
        driveTrain.addVisionMeasurement(rightPose.pose, rightPose.timestampSeconds);
      }
    }
  }

  public boolean IsRobotHeadingReset() {
    return IsRobotHeadingReset();
  }

  public void setRobotHeadingReset(boolean value) {
    robotHeadingReset = value;
  }

  //   public void disabledDeviations(Drive driveTrain) {
  //     driveTrain.setVisionMeasurementStdDevs(VecBuilder.fill(0.05, 0.05, 0.3));
  //   }

  //   public void autonDeviations(Drive drivetrain) {
  //     drivetrain.setVisionMeasurementStdDevs(VecBuilder.fill(0.1, 0.1, 9999999));
  //   }

  //   public void enabledDeviations(Drive driveTrain) {
